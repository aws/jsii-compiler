...
test('produces correct output', () => {
  const result = compileJsiiForTest(MULTI_FILE_EXAMPLE, undefined /* callback */, { stripDeprecated: true });
...
describe('stripDeprecatedAllowList', () => {
  test('strips all if all FQNs are present in the allowList', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'jsiideprecatedremover'));
    ...
    const resultWithoutAllowList = compileJsiiForTest(MULTI_FILE_EXAMPLE, undefined /* callback */, {
      stripDeprecated: true,
    });
    const resultWithAllowList = compileJsiiForTest(MULTI_FILE_EXAMPLE, undefined /* callback */, {
      stripDeprecated: true,
      stripDeprecatedAllowListFile,
    });
  ...
  test('strips none if the allowList is empty', () => {
    ...
    const resultNoStrip = compileJsiiForTest(MULTI_FILE_EXAMPLE, undefined /* callback */, { stripDeprecated: false });
    const resultStripEmpty = compileJsiiForTest(MULTI_FILE_EXAMPLE, undefined /* callback */, {
      stripDeprecated: true,
      stripDeprecatedAllowListFile,
    });
  ...
  test('strips only allowlisted items if subset is present', () => {
    ...
    fs.writeFileSync(
      stripDeprecatedAllowListFile,
      ['testpkg.IDeprecatedInterface', 'testpkg.SomeEnum#VALUE_DEPRECATED', 'testpkg.GrandChild#deprecatedMethod'].join(
        '\n',
      ),
      'utf8',
    );

    const result = compileJsiiForTest(MULTI_FILE_EXAMPLE, undefined /* callback */, {
      stripDeprecated: true,
      stripDeprecatedAllowListFile,
    });
