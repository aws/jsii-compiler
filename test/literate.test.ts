import { includeAndRenderExamples, typescriptSourceToMarkdown } from '../src/literate';
...
test('can switch on and off', () => {
  assertRendersTo(
    ['const x = 1;', '/// !show', 'console.log(x);', '/// !hide', 'console.log("It worked")'],
    ['```ts', 'console.log(x);', '```'],
  );
});
...
test('common whitespace in a single block gets stripped', () => {
  assertRendersTo(
    ['const x = 1;', 'if (x) {', '    /// !show', '    console.log(x);', '    /// !hide', '}'],
    ['```ts', 'console.log(x);', '```'],
  );
});
...
test('inline markdown with indentation still gets rendered', () => {
  assertRendersTo(
    ['const x = 1;', 'if (x) {', '    /// This is how we render x', '    console.log(x);', '}'],
    ['```ts', 'const x = 1;', 'if (x) {', '```', 'This is how we render x', '```ts', '    console.log(x);', '}', '```'],
  );
});

test('subsequent code blocks get joined', () => {
  assertRendersTo(
    ['/// !show', 'let x = 1;', '/// !hide', 'x += 1;', '/// !show', 'console.log(x);', '/// !hide'],
...
test('can add inline MarkDown', () => {
  assertRendersTo(
    ...
    ['```ts', 'const x = 1;', '```', 'This is how we print x', '```ts', 'console.log(x);', '```'],
  );
});

test('can do example inclusion', () => {
  const inputMarkDown = ['This is a preamble', '[included here](test/something.lit.ts)', 'This is a postamble'];
